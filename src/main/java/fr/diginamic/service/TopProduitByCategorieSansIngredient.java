...
/**
 * Représente la recherche des 10 premiers produits les mieux notées d'une catégorie sans un ingrédient donné
 * 
 * @author Marjory PRIN
 */
public class TopProduitByCategorieSansIngredient extends MenuService {
	...
	public void traiter(Scanner scanner) {
		...
		// Demande du nom de la catégorie et l'ingrédient à exclure à l'utilisateur
		System.out.println("\n/////////////////////////////////////////\n");
		...
		TypedQuery<Produit> query = em.createQuery("SELECT DISTINCT p FROM Produit p JOIN p.categorie c WHERE c.nom=:param1 "
				+ "AND p.id NOT IN(SELECT DISTINCT p.id FROM Produit p JOIN p.categorie c JOIN p.ingredients i WHERE c.nom=:param1 AND i.nom=:param2) "
				+ "ORDER BY p.nutrition.nutriscore", Produit.class);
		query.setParameter("param1", scan1);
		query.setParameter("param2", scan2);
		List<Produit> produits = query.getResultList();		
		
		// Affichage des produits avec le meilleur nutriscore pour la catégorie sans un ingrédient donnée
		if (produits.size()==0) {
			System.out.println("\nCette catégorie n'existe pas ou il n'y a pas de produit dans cette catégorie sans l'ingrédient demandé !");
		} 
